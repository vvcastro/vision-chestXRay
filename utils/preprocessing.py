# In this file the main pre-processing functions
import numpy as np
import cv2
import os
...
class DataProcessor:
    ...
    def init_configs(self, noisy=53, inverted=65, blurred=1000):
    ...
    def extract_relevant_patches(self, image):
        ...
        # take top corners
        crop_height, crop_width = image.shape[0] // 8, image.shape[1] // 8
        ...
        crop_height, crop_width = image.shape[0] // 4, image.shape[0] // 2
        crops.append( image[
            -crop_height:,
            img_center_x - (crop_width // 2):img_center_y + (crop_width // 2)
        ] )
    ...
    def store_in_memory(self, image_name, processed_image):
        """ 
        Store the pre-processed image with the original name
        but in the Class name foldr.
        """
        output_path = os.path.join(self.output_path, image_name)
        try:
